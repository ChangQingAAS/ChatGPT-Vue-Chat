<template>
    <div>
        <section :class="showSelBox>0?'chatlist-bottom-collapse':'chatlist-bottom'" class="chatlist">
            <mt-loadmore ref="loadmore" :top-method="loadTop" top-drop-text="释放加载"
                         top-pull-text="加载更多" @top-status-change="handleTopChange">
                <ul>
                    <div v-for="item in records" :key="item.type">
        ...
        <section class="foot">
            ...
            <span class="btn-face" v-on:click="showSelBox=showSelBox==1?0:1"><i aria-hidden="true"
                                                                                class="fa fa-smile-o"></i></span>
            <span class="btn-plus" v-on:click="showSelBox=showSelBox==2?0:2"><i
                :class="showSelBox==2?'fa-minus-circle':'fa-plus-circle'" aria-hidden="true"
                class="fa"></i></span>
            <span class="btn btn-send" v-on:click="sendMsg">发送</span>
            <section :class="showSelBox>0?'show':'hide'" class="selbox">
                    ...
                    <mt-swipe :auto="0" :continuous="false">
                        <mt-swipe-item v-for="n in Math.ceil(EXPS.length/18)" :key="n">
                            <li v-for="(item, index) in getEXP(n,18)" :key="index">
                                <img :key="index" :data="item.code" :src="'static/emotion/'+item.file"
                                     alt=""
    ...
    data () {
        return {
            ...
            records: [{
                ...
                name: '用户',
                content: '我想和BingChatGPT聊天。'
            }, {
                type: 2,
                time: util.formatDate.format(new Date(), 'yyyy-MM-dd hh:mm:ss'),
                name: 'BingChatGPT',
                content: '欢迎进入BingChatGPT的聊天！'
            }],
            // 表情
            EXPS: [
    ...
    methods: {
        ...
        replaceFace: function (con) {
            ...
            for (var i = 0; i < exps.length; i++) {
                ...
                con = con.replace(exps[i].reg, '<img src=' + exps[i].file + '"../../public/static/emotion/"  alt="" />')
            }
            return con
        },
